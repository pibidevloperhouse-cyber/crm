...
export default function InvoicePreview({ dealId, children }) {
  ...
  const subtotal = deal?.products?.reduce((sum, _, i) => {
    return (
      sum +
        ...
        <SheetContent side="right" className="w-full sm:max-w-4xl overflow-y-auto">
          ...
          {!deal ? (
            ...
            <div className="space-y-6">
              <div id="invoice-pdf-content" className="bg-white w-full p-8 shadow-md">
                {/* Header Section */}
                  ...
                  <div className="flex justify-between items-start mb-6">
                    ...
                    <div className="text-right">
                      ...
                      <div className="mt-3 text-black text-sm space-y-1">
                        <p>
                          <strong>Date:</strong> {deal?.created_at
                            ? new Date(deal.created_at).toLocaleDateString()
                            : 'N/A'}
                        </p>
                        <p>
                          <strong>Invoice #:</strong> INV-{String(dealId).slice(-6)}
                        </p>
                        <p>
                          <strong>Customer ID:</strong> {deal?.customer_id || 'ABC12345'}
                        </p>
                        <p>
                          <strong>Purchase Order #:</strong> {deal?.po_number || '12345678'}
                        </p>
                        <p>
                          <strong>Payment Due by:</strong> {deal?.valid_until
                ...
                {template?.header_image_url && (
                  <div className="flex justify-between mt-4">
                    ...
                    <div className="text-right">
                      <h2 className="text-3xl text-blue-800 font-bold mb-2">INVOICE</h2>
                      <table className="text-sm border border-gray-700">
                        <tbody>
                          ...
                          <tr>
                            <td className="border border-gray-700 px-2 font-semibold">INVOICE #</td>
                            <td className="border border-gray-700 px-2">INV-{String(dealId).slice(-6)}</td>
                          </tr>
                          <tr>
                          ...
                          <tr>
                            <td className="border border-gray-700 px-2 font-semibold">PAYMENT DUE</td>
                ...
                <div className="grid grid-cols-2 gap-6 mb-6">
                  ...
                  <div>
                    ...
                    <div className="mt-2 text-sm text-gray-700 space-y-1">
                      ...
                      <p>{deal?.ship_to_company || deal?.customer_company || '[Company Name]'}</p>
                      <p>{deal?.ship_to_address || deal?.address || '[Street Address]'}</p>
                      <p>{deal?.ship_to_city || deal?.city || '[City, ST ZIP Code]'}</p>
                ...
                {template?.footer_image_url ? (
                  ...
                  <div className="text-center mt-8 text-sm text-gray-600">
                    <p>Make all checks payable to {deal?.title || 'My Company name'}</p>
                    <p className="font-semibold mt-2">Thank you for your business!</p>
                    <p className="mt-2">
                      Should you have any enquiries concerning this invoice, please
                      contact {deal?.salesperson || 'John Doe'} at {deal?.email || 'email@company.com'}
                    </p>
                    <p className="mt-2">{deal?.address || '111 Street, Town/City, County, ST, 00000'}</p>
                  </div>
                )}
              </div>
              ...
              <div className="flex gap-3 justify-end">
                ...
                >
                  ...
                  {generatingPDF ? 'Generating PDF...' : 'Download Invoice as PDF'}
                </Button>
              </div>
            </div>
          )}
