...
export default function LeadCardMobile({ lead, onChange, fetchLeads, fetchDeals }) {
    ...
    <>
      ...
      >
        <div className="flex items-center justify-between gap-2 mb-2">
          ...
          <span className={`w-2 h-2 rounded-full flex-shrink-0 ${STATUS_DOT[lead.status] || "bg-slate-400"}`} />
        </div>
          ...
          <div className="flex items-center justify-between px-4 py-3 border-b bg-white dark:bg-slate-900 flex-shrink-0">
            <div className="flex items-center gap-2.5 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <Badge className={`${STATUS_COLOR[lead.status]} text-white border-0 text-[9px] h-4 px-1.5`}>
                  {lead.status}
                </Badge>
              </div>
            </div>
          ...
          <div className="flex-1 overflow-y-auto bg-slate-50/50 dark:bg-slate-950">
            ...
            {activeTab === "info" && (
              <div className="p-4 space-y-1">
                ...
                {lead.description && (
                  <div className="pt-3">
                    <p className="text-[10px] text-slate-400 uppercase tracking-widest mb-2">Description</p>
            ...
            {activeTab === "flow" && (
              <div className="p-4">
                <p className="text-[10px] font-bold uppercase text-slate-400 tracking-widest mb-4">Lead Pipeline</p>
                        ...
                        className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl border transition-all active:scale-[0.98] ${
                          isCurrent
                            ...
                            : isCompleted
                              ...
                              : "bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-400 hover:border-teal-300"
                        }`}
                      >
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent className="fixed bottom-0 left-0 right-0 top-auto translate-x-0 translate-y-0 max-w-full w-full h-[92vh] rounded-t-2xl rounded-b-none flex flex-col p-0 gap-0 border-0 shadow-2xl">
          ...
          <div className="flex items-center justify-between px-4 py-3 border-b bg-white dark:bg-slate-900 flex-shrink-0">
            <h2 className="text-base font-bold text-slate-900 dark:text-white">Edit Lead</h2>
            <button
      ...
      <Dialog open={emailOpen} onOpenChange={setEmailOpen}>
        <EmailTemplate type="Leads" id={lead.id} email={lead.email} />
      </Dialog>
      ...
      <Dialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <DialogContent className="mx-4 max-w-sm rounded-2xl">
          ...
          <DialogFooter className="flex gap-2 sm:gap-0">
            ...
            <Button variant="destructive" className="flex-1" onClick={handleDeleteLead}>Delete</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
