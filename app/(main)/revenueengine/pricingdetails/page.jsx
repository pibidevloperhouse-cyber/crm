...
export default function PricingDetailsPage() {
      ...
      if (!allDealsError && allDeals) {
        ...
        allDeals.forEach((deal) => {
          ...
          if (originalVal > 0) {
            ...
            if (!loyaltyMap[deal.email].discounts)
              loyaltyMap[deal.email].discounts = [];
            loyaltyMap[deal.email].discounts.push(dpt);
          }

          if (deal.status === "Closed-won") {
            loyaltyMap[deal.email].won_deals =
              (loyaltyMap[deal.email].won_deals || 0) + 1;
          }
  ...
  useEffect(() => {
    ...
    if (email) {
      ...
      console.warn(
        "No user email found in localStorage (session or user keys)",
      );
      setIsLoading(false);
    }
  }, []);
        ...
        if (deal.id === dealId) {
          ...
          if (perProductFields.includes(field)) {
            ...
            while (newFieldArray.length <= productIndex) {
              ...
              else if (field === "auto_discount_status")
                newFieldArray.push("pending");
              else if (
                field === "labour_cost" ||
  ...
  const getCalculatedAIDiscount = (deal, index) => {
    ...
    const anchorOriginalPriceDiscount = parseFloat(
      Math.min(
        60,
        bundleDiscount +
  ...
  const getTotals = (deal) => {
    ...
    if (taxScope === "Line") {
      ...
      deal.products.forEach((productName, index) => {
        const { lineBeforeTax, lineTaxAmount } = calculateLineMetrics(
          deal,
          productName,
          index,
        );
      ...
      let rawSubtotal = 0;
      deal.products.forEach((productName, index) => {
        const { lineBeforeTax } = calculateLineMetrics(
          deal,
          productName,
          index,
        );
  ...
  const calculateLineItem = (deal, productName, index) =>
    calculateLineMetrics(deal, productName, index).lineBeforeTax;
                    ...
                    <div className="flex-grow">
                      <div className="flex items-center gap-3 mb-1">
                        ...
                        <div className="flex gap-2">
                          <Badge
                            className={`${deal.approved ? "bg-green-100 text-green-700" : "bg-amber-100 text-amber-700"} border-none text-[10px]`}
                          >
                            {deal.approved ? "APPROVED" : "DRAFT"}
                          </Badge>
                          {/* Integrated Loyalty Badge directly in Header */}
                          <Badge
                            className={`${getLoyaltyInfo(deal.email).color} border-none px-2 py-0.5 text-[9px] font-black uppercase tracking-wider shadow-sm`}
                          >
                ...
                <CardContent className="p-6">
                    ...
                    <Table>
                      ...
                      <TableBody>
                        {deal.products.map((productName, index) => {
                          ...
                          const statusObj = deal.auto_discount_status || [];
                          let status =
                            statusObj.length > index
                              ? statusObj[index]
                              : "pending";
                          const userValStr = String(
                            deal.user_discount?.[index] || "",
                          );
                          const userValNum = parseFloat(
                            userValStr.replace("%", ""),
                          );
                          ...
                          const displayedValue = isAutoApplied
                            ? aiInfo.combinedTargetDiscount
                            : isNaN(userValNum)
                              ? "0"
                              : userValNum;
                          const netPrice =
                            originalPrice * (1 - Number(displayedValue) / 100);
                          ...
                          return (
                            <TableRow key={`${deal.id}-${index}`}>
                              ...
                              <TableCell>
                                <Select
                                  value={
                                    deal.pricing_strategy?.[index] || "AI Auto"
                                  }
                                  onValueChange={(val) =>
                                    handleStrategyChange(deal.id, index, val)
                                  }
                                ...
                                <div className="flex flex-col items-center gap-1">
                                  <TooltipProvider>
                                    <Tooltip>
                                      ...
                                      <TooltipContent
                                        side="bottom"
                                        className="text-[10px] w-64 p-4 bg-slate-950 text-white border-slate-800 shadow-xl rounded-xl"
                                      >
                                  ...
                                  <TooltipProvider>
                                    <Tooltip>
                                      ...
                                      <TooltipContent
                                        side="bottom"
                                        className="w-[320px] p-6 bg-slate-950/80 backdrop-blur-2xl border border-slate-800/60 shadow-[0_20px_40px_-15px_rgba(0,0,0,0.7)] rounded-2xl ring-1 ring-white/5"
                                      >
                                        <div className="space-y-5">
                                          ...
                                          <div className="space-y-4">
                                              ...
                                              <div className="space-y-2">
                                                <div className="flex justify-between items-start p-3 bg-white/[0.03] rounded-xl border border-white/5 hover:bg-white/[0.05] transition-colors">
                                                  <div className="pr-4">
                                                    ...
                                                    <p className="text-[9px] text-slate-400 leading-snug">
                                                      Engine detects a{" "}
                                                      {aiInfo.intentScore >= 75
                                                        ? "high"
                                                        : "moderate"}{" "}
                                                      purchase likelihood based
                                                      on cart density &
                                                      historical win-loss
                                                      vectors.
                                                    </p>
                                                ...
                                                <div className="flex justify-between items-start p-3 bg-white/[0.03] rounded-xl border border-white/5 hover:bg-white/[0.05] transition-colors">
                                                  <div className="pr-4">
                                                    ...
                                                    <p className="text-[9px] text-slate-400 leading-snug">
                                                      Recognizes established
                                                      business relationship
                                                      unlocking top-tier loyalty
                                                      provisioning limits.
                                                    </p>
                                                  </div>
                                                  <span className="text-xs font-mono text-emerald-400 font-bold bg-emerald-400/10 px-1.5 py-0.5 rounded shadow-sm">
                                                    +{aiInfo.loyaltyDiscount}%
                                                  </span>
                                                </div>
                                                ...
                                                {aiInfo.bundleDiscount > 0 && (
                                                  <div className="flex justify-between items-start p-3 bg-white/[0.03] rounded-xl border border-white/5 hover:bg-white/[0.05] transition-colors">
                                                    ...
                                                    <span className="text-xs font-mono text-purple-400 font-bold bg-purple-400/10 px-1.5 py-0.5 rounded shadow-sm">
                                                      +{aiInfo.bundleDiscount}%
                                                    </span>
                                                  </div>
                                                )}
                                            ...
                                            <div className="pt-4 border-t border-white/10 flex justify-between items-center bg-slate-900/40 -mx-6 px-6 -mb-6 rounded-b-2xl pb-5">
                                              <div>
                                                <span className="text-[10px] font-bold uppercase tracking-[0.15em] text-slate-400 block mb-0.5">
                                                  Final Computed Discount
                                                </span>
                                                <span className="text-[9px] text-slate-500">
                                                  Applied to line before tax
                                                </span>
                                              </div>
                                              <span className="text-xl font-black text-white px-2 py-1 bg-white/10 rounded-lg backdrop-blur-sm border border-white/10 shadow-inner">
                                                -{displayedValue}%
                                              </span>
                                            </div>
                                          </div>
                  ...
                  <div className="mt-4 mb-2 p-2 rounded-lg border border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/40 flex flex-wrap items-center gap-4">
                    <div className="flex items-center gap-2 border-r border-slate-200 dark:border-slate-800 pr-4">
                      ...
                      <span className="text-sm font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest whitespace-nowrap">
                        Tax
                      </span>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-4 grow">
                      <div className="flex items-center gap-2">
                        ...
                        <Select
                          value={deal.tax_region_type || "None"}
                          onValueChange={(val) =>
                            handleChange(deal.id, "tax_region_type", null, val)
                          }
                        >
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Select
                          value={deal.tax_scope || "Transaction"}
                          onValueChange={(val) =>
                            handleChange(deal.id, "tax_scope", null, val)
                          }
                        >
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Select
                          value={deal.tax_inclusion || "Exclusive"}
                          onValueChange={(val) =>
                            handleChange(deal.id, "tax_inclusion", null, val)
                          }
                        >
                          <SelectTrigger className="h-8 text-sm bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 px-2 font-medium min-w-[130px]">
                            <SelectValue />
                      ...
                      {(!deal.tax_scope || deal.tax_scope === "Transaction") &&
                        deal.tax_region_type !== "None" && (
                          <div className="flex items-center gap-3 min-w-[160px] grow max-w-[240px]">
                            ...
                            <Slider.Root
                              ...
                              step={0.5}
                              onValueChange={([val]) =>
                                handleChange(deal.id, "tax_rate", null, val)
                              }
                            >
                            ...
                            <span className="text-sm font-mono font-bold text-slate-700 dark:text-slate-200 min-w-[40px] text-right">
                              {deal.tax_rate ?? 0}%
                            </span>
                          </div>
                        )}
                    ...
                    <div className="ml-auto pl-4 border-l border-slate-200 dark:border-slate-800 flex flex-col items-end">
                      ...
                      <span className="text-sm font-black text-slate-900 dark:text-white tabular-nums">
                        {products[0]?.currency || "$"}
                        {getTotals(deal).totalTax.toFixed(2)}
                      </span>
                    </div>
                  </div>
                    ...
                    <div className="flex gap-6 items-end">
                      <div className="text-right">
                        ...
                        <p className="text-base font-bold text-slate-700 dark:text-slate-300">
                          {products[0]?.currency || "$"}
                          {getTotals(deal).subtotalBeforeTax.toFixed(2)}
                        </p>
                      </div>
                      <div className="text-right">
                        <p className="text-[10px] text-slate-500 uppercase font-bold tracking-tight mb-0.5">
