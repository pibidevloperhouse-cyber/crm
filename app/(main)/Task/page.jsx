...
export default function TaskPage() {
  ...
  const [leads, setLeads] = useState([]); // Array fueling the client modal menu
  ...
  const fetchLeads = async () => {
    try {
      ...
      console.error("Error fetching leads details:", err.message || err);
    }
  };
  ...
  // Execution Hook safely calls fetchLeads and fetchTasks once defined above
  useEffect(() => {
    if (currentUserEmail) {
    ...
    >
      ...
      <div className="flex flex-col lg:flex-row flex-1 min-h-0">
        ...
        >
              ...
              <div className="grid grid-cols-7 gap-y-1">
                ...
                {Array.from({ length: daysInMonth }).map((_, i) => {
                  ...
                  return (
                    ...
                    >
                      <span className="text-xs font-semibold" style={{ color: isSel ? "#fff" : isToday ? "#0ea5e9" : dm ? "#94a3b8" : "#475569" }}>
                        {day}
                      </span>
                      {dayTasks.length > 0 && (
                        <div className="flex gap-0.5 mt-0.5">
                          {dayTasks.slice(0, 3).map((t) => (
                            <div key={t.id} className="w-1 h-1 rounded-full" style={{ background: isSel ? "rgba(255,255,255,0.85)" : t.color }} />
                          ))}
                        </div>
                      )}
                    </button>
          ...
          <button
            ...
            style={{
              ...
              borderTop: calendarOpen ? (dm ? "1px solid #21262d" : "1px solid #e2e8f0") : "none",
            }}
          >
            {calendarOpen ? (
              <div className="flex items-center gap-1.5 px-5 py-1 rounded-full" style={{ background: dm ? "rgba(14,165,233,0.15)" : "rgba(14,165,233,0.12)" }}>
                <ChevronLeft size={14} className="rotate-90" style={{ color: dm ? "#38bdf8" : "#0284c7" }} />
                <span className="text-xs font-semibold" style={{ color: dm ? "#38bdf8" : "#0284c7" }}>Hide Calendar</span>
                <ChevronLeft size={14} className="rotate-90" style={{ color: dm ? "#38bdf8" : "#0284c7" }} />
              </div>
            ) : (
              <>
        ...
        <div className={`flex flex-col flex-1 min-h-0 min-w-0 ${pageBg}`}>
          <div className={`flex-shrink-0 ${panelBg} border-b ${border} px-4 sm:px-6 py-3`}>
            <h2 className={`text-sm sm:text-base font-bold ${txtPrimary} leading-tight`}>{selectedDateLabel}</h2>
            <p className={`text-xs ${txtMuted} mt-0.5`}>{selectedTasks.length} task{selectedTasks.length !== 1 ? "s" : ""}</p>
          </div>

          <div className={`flex-shrink-0 ${panelBg} border-b ${border} px-4 sm:px-6 py-3`}>
            <div className="flex flex-col sm:flex-row sm:items-center gap-2">
              <div className={`flex items-center gap-2 px-3 py-2 rounded-lg border flex-1 sm:max-w-xs ${dm ? "bg-[#161b22] border-[#30363d]" : "bg-slate-100 border-slate-200"}`}>
              ...
              <div className="flex gap-1 flex-wrap">
                {["all", "Meeting", "Call", "Task", "Follow up"].map((cat) => (
                  <button
                    ...
                    style={{
                      background: filterCategory === cat ? "linear-gradient(135deg,#0ea5e9,#14b8a6)" : dm ? "#161b22" : "#f1f5f9",
          ...
          <div className="flex-1 overflow-y-auto overscroll-contain p-4 sm:p-6 space-y-3">
            {loading ? (
              ...
              <div className={`flex flex-col items-center justify-center h-40 rounded-xl gap-3 border border-dashed ${dm ? "bg-[#0d1117] border-[#30363d]" : "bg-white border-slate-300"}`}>
                <Calendar size={28} className={txtMuted} />
                <p className={`text-sm ${txtMuted}`}>No tasks on this day</p>
                <button onClick={() => openModal("add")} className="text-xs px-4 py-1.5 rounded-lg font-semibold text-white" style={{ background: "linear-gradient(135deg,#0ea5e9,#14b8a6)" }}>
                  + Add Task
                </button>
              </div>
            ) : (
              selectedTasks.map((task) => (
                <div key={task.id} className={`rounded-xl border p-4 sm:p-5 shadow-sm hover:shadow-md transition-all duration-200 ${dm ? "bg-[#0d1117] border-[#21262d]" : "bg-white border-slate-200"}`}>
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <h3 className={`text-sm sm:text-base font-bold ${txtPrimary}`}>{task.title}</h3>
                    {task.status === "closed" && (
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full flex-shrink-0 ${dm ? "bg-slate-800 text-slate-500" : "bg-slate-100 text-slate-400"}`}>Closed</span>
                    )}
                  </div>
                  ...
                  {task.client_name && (
                    <div className="flex items-center gap-2 mb-1.5">
                      ...
                      <span className={`text-xs sm:text-sm font-medium ${dm ? "text-sky-400" : "text-sky-600"}`}>{task.client_name}</span>
                    </div>
                  )}

                  {task.user_email && (
                    <div className="flex items-center gap-2 mb-3">
                      <span className={`text-[11px] px-2 py-0.5 rounded-full font-medium ${dm ? "bg-teal-900/40 text-teal-400" : "bg-teal-50 text-teal-600"}`}>{task.user_email}</span>
                    </div>
                  )}

                  <p className={`text-xs sm:text-sm ${txtMuted} mb-3 pb-3 border-b ${divider}`}>{task.description}</p>

                  <div className="flex gap-2 flex-wrap">
                    <button onClick={() => openModal("delete", task)} className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg transition-colors ${dm ? "text-red-400 bg-red-400/10 hover:bg-red-400/20" : "text-red-600 bg-red-50 hover:bg-red-100"}`}>
                      <Trash2 size={13} /> Delete Task
                    </button>
                    <button onClick={() => openModal("edit", task)} className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg transition-colors ${dm ? "text-amber-400 bg-amber-400/10 hover:bg-amber-400/20" : "text-amber-600 bg-amber-50 hover:bg-amber-100"}`}>
                      <Edit size={13} /> Update Task
                    </button>
                    {task.status !== "closed" && (
                      <button onClick={() => openModal("close", task)} className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg transition-colors ${dm ? "text-emerald-400 bg-emerald-400/10 hover:bg-emerald-400/20" : "text-emerald-600 bg-emerald-50 hover:bg-emerald-100"}`}>
